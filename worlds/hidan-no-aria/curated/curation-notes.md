# Curation notes - 绯弹的亚里亚

## 人工判断
- 使用 `manifest.json` 确认来源规模与 `src-hidan-no-aria-001`；按武侦制度、人物血脉、组织冲突手工整理。
- 武侦技能按现代武装+校园任务处理；超常血脉按特殊能力补充，不替代枪械战。

## 冲突与低置信
- 轻小说后续组织数量多，本次只归纳伊・幽等核心势力。
- 人名译法（亚里亚/婭莉婭、蕾姬/Reki）按常见中文与源题并存。

## 来源路径
- `campaigns/world-library/worlds/hidan-no-aria/manifest.json`
- `campaigns/world-library/worlds/hidan-no-aria/characters/characters.json`
- `campaigns/world-library/worlds/hidan-no-aria/world/world.json`
